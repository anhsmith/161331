...
It is important to distinguish between these two procedures (esting hypotheses and model selection) because the goals and methods are different.

Testing hypotheses is about weighing the evidence for the **existence** of an effect. More correctly, it is about weighing evidence **against** a null hypothesis where the hypothesised effect is zero. With hypothesis testing, we put the burden of evidence on the data to contradict the null hypothesis. If the test is inconclusive, then we stick with the null (though we do not assert the null is true). Basically, the null stands until there is data show otherwise. The key tools for testing hypotheses are p-values (or, sometimes, showing a confidence interval that doesn't contain zero). 
...
In our case, we wish to know if the different fertilisers affect yield, and whether the effects of fertiliser differ among varieties (i.e., the interaction between fertiliser and yield). As always, we begin by examining the interaction term.

With simple ANOVA-type models, we often just leave all the terms in the model, even ones that aren't statistically significant. But when it is clear that a term is negligible, it is prudent to remove it. 

### Testing the random interaction between fertiliser and variety

Let's now test the hypothesis that the variance component for the interaction term is zero. 
...
```
Now, that's convincing! The average yields are definitely different among varieties.

*Wait, what?*

It is worthwhile stopping for a moment and asking: why did we test this hypothesis? There is actually no particular reason to do so. What have we done? What does the null hypothesis represent?

The null hypothesis was that all varieties of potato have exactly the same yield, on average. In other words, no variety has an average yield any different to any other variety. Does that sound realistic? I think not. It's a bit of a 'straw-man' hypothesis, really. 
...
Now let's do the same for variety - calculate the mean yield for each variety, marginalised over the fertiliser types. 
