...
```{r, echo=TRUE}
tmpf <- function(x) { round(c(Mean=mean(x),SD=sd(x)),3) }
aggregate(bright~operator, data=pulp, FUN=tmpf)
...
```{r, echo=TRUE}
with(pulp, plot(operator,bright))
```
...
```{r, echo=TRUE}
with(pulp, plot(operator,bright))
with(pulp, points(jitter(as.numeric(operator)), bright, col="red"))
```
