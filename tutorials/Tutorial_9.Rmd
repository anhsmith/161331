...
There is one numerical response and one categorical predictor.

```{r, echo=TRUE}
table(pulp$operator)
```

This is a small dataset: 5 measurements from each of 4 operators.
...
We can also use the `anova` function to apply a Likelihood Ratio Test, than tests the model with `operator` against a 'null' model with only an intercept (i.e., just estimating a single, overall mean).
...
These results indicate a significant effect of `operator`, when treated as a fixed effect. We have evidence against the null hypothesis that all the means are the same. 

## Linear model with random effect

### Calculating the variance component with the ANOVA method
...
Recall the following formula from the lecture.
...
We can test whether a model with an overall intercept and a random effect of `operator` explains significantly more variation in `bright` than a model with just an intercept and no random effect of `operator`. 

To do this test, we'll make the two models: a reference model that includes the random factor `operator`, and a nested model that does not. Then, we'll do a likelihood ratio test for these two models.
