...
$y_{ij}$ is the pains score from Time $i$, Subject $j$,

$\alpha_{i}$ is the effect of Time $i$,

$b_{j}$ is the effect of Subject $j$,
...
predf <- avg_predictions(lmer1, by="Time")
```
...
It looks like the average pain increased slightly more from 5 and 7 PM than from 5 and 3 PM. Although, with such a small dataset, this may be driven solely by Subject 4, who seemed to be feeling unusually good at 5 PM.
...
ggplot(predall) +
  ...
  geom_pointrange(data = predf, 
             aes(x=Time, y=estimate, ymin=conf.low, ymax=conf.high), 
             inherit.aes = F, size = 1.1
             ) 
